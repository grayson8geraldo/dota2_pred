"""
Main prediction interface for pre-match Dota 2 predictions.

Ties together data collection, feature engineering, model prediction,
...
class MatchPredictor:
    """High-level interface for predicting Dota 2 matches (pre-match only)."""

    CACHE_TTL = 3600       # 1 hour — full team data
    MATCHES_TTL = 14400    # 4 hours — match history only
    ...
    def get_team_data(self, team_id: int, force_refresh: bool = False) -> dict:
        """Get team data with smart caching and circuit breaker."""
        ...
        if cached and age >= self.MATCHES_TTL and age < self.CACHE_TTL * 24:
            ...
            if fresh_matches is not None:
                ...
                if self._api_failures >= 3:
                    logger.warning("API circuit breaker OPEN: using cached data only.")
                return cached
    ...
    def refresh_teams_matches(self, team_ids: list[int]):
        ...
        for tid in stale_ids:
            if self._api_failures >= 3:
                logger.warning(f"Circuit breaker tripped after refreshing {refreshed} teams")
                break
    ...
    def _load_teams_list(self) -> list[dict]:
        """Load the /teams list from cache or API."""
        ...
        if os.path.exists(teams_path):
            try:
                with open(teams_path, "r") as f:
                    teams_list = json.load(f)
                logger.warning(f"API failed, using stale disk cache ({len(teams_list)} teams)")
                return teams_list
        ...
        if self.team_cache:
            ...
            if teams_list:
                logger.warning(f"Built teams list from team_cache ({len(teams_list)} teams)")
                os.makedirs(config.MODEL_PATH, exist_ok=True)
                try:
                    with open(teams_path, "w") as f:
                        json.dump(teams_list, f)
        ...
        logger.error("No teams data available! Run 'python main.py refresh'")
        return []

    def find_team_by_name(self, name: str) -> Optional[dict]:
        """Search for a team by name using fuzzy matching."""
        ...
        if best_score >= 80 and best_team:
            logger.debug(f"Fuzzy matched '{name}' -> '{best_team.get('name')}' (score={best_score})")
            return best_team
    ...
    def predict_match(
        ...
        """Predict the outcome of a match between two teams (pre-match)."""
        ...
        breakdown = {
            ...
            "player_strength": {
                "radiant_wr": f"{f.get('rad_player_winrate', 0.5):.1%}",
                "dire_wr": f"{f.get('dire_player_winrate', 0.5):.1%}",
                "rad_hero_pool": f"{f.get('rad_hero_pool', 0.3):.0%}",
                "dire_hero_pool": f"{f.get('dire_hero_pool', 0.3):.0%}",
            },
            "data_freshness": {
    ...
    def predict_today_matches(self, min_rating: int = 0, show_all: bool = False) -> list[dict]:
        """
        Predict matches scheduled for today (pre-match only).

        Sources:
        1. OpenDota /proMatches (paginated) — recently completed today
        2. Upcoming match API (Liquipedia-based) — scheduled matches
        """
        ...
        # ---- Source 1: Pro matches (paginated, today only) ----
        ...
        # ---- Source 2: Upcoming scheduled matches ----
        ...
        for um in upcoming:
            ...
            # For pre-match: treat LIVE from schedule as UPCOMING
            if status == "LIVE":
                status = "UPCOMING"

            best_of = um.get("best_of")
            bo_to_st = {1: 0, 2: 3, 3: 1, 5: 2}
            series_type = bo_to_st.get(best_of, 1)
        ...
        if not show_all:
            ...
            logger.info(f"  After rating filter (>= {min_rating}): {len(filtered)}/{len(match_list)}")
            match_list = filtered

        # ---- Sort by avg rating descending ----
        match_list.sort(key=lambda e: -e.get("_avg_rating", 0))

        # ---- Refresh stale team data ----
